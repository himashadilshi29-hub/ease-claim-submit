...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[${requestId}] Calculating OPD settlement for claim: ${claimId}, user: ${auth.userId}`);
    ...
    ocrResults?.forEach(result => {
      try {
        ...
        console.error(`[${requestId}] Error parsing OCR result:`, e);
      }
    });
...
Reason: ${decisionReason}` 
    ...
    if (settlementError) {
      console.error(`[${requestId}] Error storing settlement:`, settlementError);
    }
    ...
    console.error(`[${requestId}] Error calculating settlement:`, error);
